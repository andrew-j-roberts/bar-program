...
public class QueueProvisionAndBrowse extends SampleApp {
	...
	void run(String[] args) {
		...
		try {
			...
			// CONNECT TO QUEUE
			Queue ep_queue =  JCSMPFactory.onlyInstance().createQueue("bar1-simpleQueueBrowser");
			...
			for (int i = 1; i <= 10; i++) {
				...
				m.writeAttachment(bytes);
				if((i % 2) == 0) {
					m.setCos(User_Cos.USER_COS_1);
				} else {
					m.setCos(User_Cos.USER_COS_3);
				}
				try{
			...
			do {
				...
				if (rx_msg != null) {
					rx_msg_count++;
					System.out.println("Browser got message: " + rx_msg);
				}
			} while (rx_msg != null);
