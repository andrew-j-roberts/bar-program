...
public class TopicToQueueMapping extends SampleApp {
    ...
    void run(String[] args) {
        ...
        try {
            ...
            /***
             *  ADD WILDCARD SUBSCRIPTION HERE FOR ALL TOPICS ON bar1/topicToQueueMapping/*
             * **/
            Topic topicSub = JCSMPFactory.onlyInstance().createTopic("bar1/topicToQueueMapping/*");
            session.addSubscription(ep_queue, topicSub, JCSMPSession.WAIT_FOR_CONFIRM);
            ...
            m.setDeliveryMode(DeliveryMode.DIRECT);  /** SEND MESSAGE AS DIRECT TO SHOW PROMOTION */
            Topic topicPub = JCSMPFactory.onlyInstance().createTopic("bar1/topicToQueueMapping/TEST");
