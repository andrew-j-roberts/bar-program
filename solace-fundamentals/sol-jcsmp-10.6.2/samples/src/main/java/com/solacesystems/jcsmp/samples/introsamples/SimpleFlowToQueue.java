    ...
    public void run(String[] args) {
        ...
        try {
            ...
            if (useDurable) {
                ...
                /**
                 * MODIFY ENDPOINT PROP TO HAVE MESSAGES QUEUED QUOTA OF 0 — INDICATING LVQ
                 * FOR DEMO WE WILL BE USING NON-DURABLE, THIS SHOULD BE RUN.
                 * **/
                EndpointProperties endpointProp = new EndpointProperties(EndpointProperties.ACCESSTYPE_EXCLUSIVE, 500000, EndpointProperties.PERMISSION_MODIFY_TOPIC, 0);
            ...
            // EDITED OUT — PUBLISH FIRST THEN CHECK TO SEE WHAT IS RECEIVED.  SHOULD ONLY BE LAST VALUE
            // Start the receiver
            // receiver.start();
            ...
            for (int i = 0; i < 10; i++) {
				...
				msg.writeAttachment(Integer.toString(i).getBytes());
            ...
            /**
             * NOW SET UP RECEIVER, QUEUE SHOULD HAVE ONLY ONE MESSAGE QUEUED UP TO BE CONSUMED
             * **/
            receiver.start();
            Thread.sleep(500);
